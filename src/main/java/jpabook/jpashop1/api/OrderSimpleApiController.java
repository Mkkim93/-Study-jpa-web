...
import jpabook.jpashop1.repository.OrderSimpleQueryDto;
import jpabook.jpashop1.repository.OrderSimpleQueryRepository;
import lombok.Data;
...
public class OrderSimpleApiController {
    ...
    public List<SimpleOrderDto> ordersV2() {
        // ORDER 2개 ?
        /**
         * N + 1 문제 (1+N) = 1 + N(2)개 (회원 N) + (배송 N)
         */
        List<Order> orders = orderRepository.findAllByString(new OrderSearch());

        List<SimpleOrderDto> result = orders.stream()
                .map(o -> new SimpleOrderDto(o))
                .collect(Collectors.toList());// dto 가 엔티티를 파라미터로 받는 것은 문제가 없음
        return result;
    }
